...
Currently configured project: `orchestra` (target: `..`).
...
3. Keep `data/orchestra/INFO.md` current when the architecture changes
   enough to affect scanner context.
...
`--project-id orchestra` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/orchestra/`.
...
data/orchestra/
  ...
  config.json            Per-project scan settings — checked into git
  SETUP.md               Agent refresh prompt — checked in, deletable
