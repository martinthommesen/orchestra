# Agent setup for `orchestra`

This is a deepsec scanning workspace. Project `orchestra` is registered
(target: `..`). `data/orchestra/INFO.md` has been filled from the current
architecture; use this file when the project shape changes enough to refresh
that context.
...
2. **Refresh `data/orchestra/INFO.md` when needed.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id orchestra
pnpm deepsec process --project-id orchestra
```
