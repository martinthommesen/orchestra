...
> Last updated: 2026-06-26 | Sprint 7 complete (First Contact: Live-Copilot Hardening) | Status: proven end-to-end against a real GitHub repo + live Copilot CLI; next program is Symphony-parity (SSH/remote workers, Linear adapter, refresh/debug endpoints)
>
...
agent via its tools — Orchestra is a scheduler/runner and tracker _reader_. A
...
| 7      | First Contact (Live-Copilot Hardening) | ✅ Done | The first end-to-end run against a **real GitHub repo + live Copilot CLI**, and the fixes first contact forced — production-hardening, not new surface. **map.ts reconciled to observed output (F2):** `result.usage` carries no token counts (the spike's token fields were phantom); the only signal is per-message `assistant.message.outputTokens`, surfaced as `usage.output_tokens` on the `AgentMessage` only; dead `AgentMessage.role` removed; pinned to scrubbed capture fixtures (no-tool **and** tool-using), spike §8 table superseded. **Agent/tracker credential decoupled (F1):** new `copilot.github_token` (`$VAR`); the runner authenticates the agent from it, **never** `tracker.api_key` — absent → the agent-token env keys are left **unset** (not `""`, defeating the executor's `{...process.env, ...env}` merge that would otherwise inherit the daemon's `GITHUB_TOKEN`) so the child uses ambient `/login`; secret-safe on `/settings` + `/api/v1/state`, discriminator-tested. **Handoff label (#79):** a re-dispatch loop on handed-off issues was config-only — `Human Review` now in `terminal_states` + the prompt applies that exact label (zero production-code change; `deriveState`/`isEligible` already stop non-active states). **Session-resume verdict (#6):** cross-process `--resume` of a _clean_ session is honored; a _mid-turn-killed_ session is unresumable and self-heals to a fresh turn — `resume_sessions` safe-to-enable but stays default-off. **Trust posture (#9):** observed `sandboxApplied: false` (agent runs unsandboxed); README "Security & trust posture" section written. Deferrals filed (#77 budget/USD, #78 live suite). Validated **live** in the cockpit (agent opened real PRs, handed off, no loop). `docs/sprint-7/{plan,progress,done}.md`.                                                        |
...
**What works (Sprint 7 complete — first-contact hardening proven against a real GitHub repo + live Copilot CLI, on the Sprint 6 web cockpit + durable Sprint 4 orchestrator + Sprint 3 observability v2 + Sprint 5 budget/restore/humanize):**

- **Proven against live Copilot (Sprint 7).** The full loop ran end-to-end, by hand, against
  `martinthommesen/orchestra-smoke` + the real `copilot` CLI — watched in the cockpit, the agent
  opened real PRs and handed issues off through its own tooling. The fixes first contact forced:
  the JSONL→`AgentEvent` mapper reconciled to **observed** output and pinned to scrubbed capture
  fixtures (no-tool + tool-using); the **agent credential decoupled from the tracker token (F1)**
  via `copilot.github_token`; a **handoff-label** convention that stops the re-dispatch loop on
  finished work (#79); the **session-resume** carry-forward resolved by observation (#6); and the
  **trust posture** documented from observed `sandboxApplied: false` (#9). Details below.
...
- **Real adapters behind the Sprint 0 ports:**
  ...
  - **Copilot subprocess runner** (`src/adapters/agent-copilot/`) — spawns the headless
    ...
    `map.ts`, **reconciled to live output and pinned to capture fixtures in Sprint 7** —
    superseding the spike's assumed shapes), and the Command `Scope` finalizer SIGTERMs the PID
    on interrupt/stall. The agent runs as **`copilot.github_token`** (or its ambient `/login`
    when absent) — **decoupled from `tracker.api_key` (F1)**: the three agent-token env keys are
    injected from `copilot.github_token` or left **unset** (never `""`, which would survive the
    executor's env merge and inherit the daemon's `GITHUB_TOKEN`); never logged.
    `layerCopilotRunner(config)`. Per `docs/sprint-0/spike-copilot.md` (§8 superseded by Sprint 7).
...
- **Observability** (`src/core/observability/`): one structured logfmt line per event with
  ...
  (per-issue last agent activity, cap 256), and **`RecentCompletions`** (rich finished ring, cap 50) — all fed by a **tee observer** that preserves the logfmt output byte-for-byte AND appends
...
- **Durability** (`src/core/persistence/`, Sprint 4): the daemon **survives a restart**. State is
  ...
  - clean start, **never a crash**. Observability rings are not persisted — they boot empty and
    ...
    **opt-in** (`persistence.resume_sessions`, default off) and **self-healing** — a stale session
    falls back to a fresh turn against the on-disk workspace (the true record of progress).
    **Sprint 7 verdict (#6):** cross-process `--resume` of a _cleanly-completed_ session is
    honored, but a session killed _mid-turn_ is unresumable (`AgentProcessExit`) and self-heals to
    a fresh turn — proven live. Safe to enable, kept default-off (narrow benefit, always fail-safe). All
...
planDispatch(...)`). It pauses **NEW** dispatch only: in-flight workers, pending retries
    ...
    _resume_ path is unreachable in production today (spend only grows, config loads once) but
...
- **Session resume is unproven against a live Copilot.** `persistence.resume_sessions` is
  ...
  self-heal path is tested). Sprint 5 surfaces _that_ a restore happened (#54), not whether session
