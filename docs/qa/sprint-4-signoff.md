...
durability invariant in the spike holds and is pinned by a test that would _fail_ if the
...
- **Concurrent shutdown during a debounced write:** `runWriter` registers the final-flush
  finalizer **before** `forkScoped` (LIFO → the writer fiber is interrupted _first_, the flush
  runs _last_); both the debounced write and the flush serialize through a `Semaphore(1)`
  (`writeLock`), and Effect releases the permit on interruption, so the flush can always acquire
  it — no deadlock, no half-write, latest state durable. The mid-write-interruption _race_ is
