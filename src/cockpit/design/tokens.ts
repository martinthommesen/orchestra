 ...
 * `tokens.css`). The render-ready status badge (glyph + label + color var) is derived once in
 * `model/fleet.ts` (`badgeOf`/`badgeForPhase`) and consumed by `StatusChip`, so color + glyph
 * parity stays structural, not duplicated.
 */
